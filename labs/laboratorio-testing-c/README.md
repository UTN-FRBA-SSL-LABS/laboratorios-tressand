...
> R: carrito_init resetea el contador de items del carrito a cero. El propósito de esto es evitar que datos basura producto de la asignación de memoria sean erroenamente interpretados como productos válidos dentro del carrito.
...
> R:Reemplazar '\_\_\_\_' por el valor esperado
...
> R: Falla. Devuelve 1 porque sí pudo agregar el producto
...
> R: El condicional debería ser < en lugar de <=
...
> R: La funcionalidad de descuento, porque solo se utiliza en los tests de integración
...
> R: El test unitario, porque el bug estaba en el cálculo del total para cualquier producto
...
> R: Es peligroso porque se puede escribir en memoria sensible a la que normalmente no se podría acceder. En un lenguaje como esos no ocurriría porque la gestión de memoria es automática.
...
> R: Los test son particularmente útiles para que las verificaciones persistan durante la evolución del repositorio. No sirve de nada testear más de una vez algo que nunca va a cambiar.
...
> R: Porque si la cantidad es 1, entonces la multiplicación que faltaba nunca habría tenido efecto igualmente. El resultado es el mismo.
